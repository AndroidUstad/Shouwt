...
extension ContextExtension on BuildContext {
  ...
  Future<void> cropImage({
    required File file,
    required Function(String) onCropped,
  }) async {
    ...
    if (croppedFile != null) {
      await onCropped(croppedFile.path);
      this.router.pop();
    }
  }
  ...
  Future<EditorImageResult> editImage(File image) async {
    final editedImage = (await Navigator.push(
      this,
      MaterialPageRoute(builder: (context) => ImageEditor(originImage: image)),
    ) as EditorImageResult);
  ...
  showOkCancelAlertDialog<T>(
    ...
    AlertDialog alert = AlertDialog(
      ...
      actions: [
        okButtonTitle
            .toButton(
                color: isDark ? AppColors.colorPrimary : AppColors.textColor)
            .toFlatButton(() {
          // Fix the issue
        ...
        LocaleKeys.cancel
            ...
            .toButton(
                color:
                    isDark ? Colors.white.withOpacity(.6) : AppColors.textColor)
            .toFlatButton(() {
          this.router.root.pop();
        })
      ],
